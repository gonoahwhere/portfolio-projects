...
const { titleCase, getTurnEmbedFields, createCellButtons, drawSudokuGrid } = require('../../commands/global/sudoku');
...
module.exports = async function handleGrid(interaction) {
    ...
    if (!gameState) {
        ...
        console.log({
            ...
            savedHostId: savedGame.hostId,
            joinedPlayers: savedGame.joinedPlayers,
    ...
    const buffer = await drawSudokuGrid(gameState.puzzle, gameState.prefilledSet, gameState.theme, null, null, gridNum, gameState.conflictSet, gameState.notes.map(s => Array.from(s)), interaction.user.id);
